...
function controller($name, $path = '')
{
	...
	if (!C('APP_USE_NAMESPACE')) {
		...
		$class .= $layer;
		//var_dump($class,$path,MODULE_NAME,C('APP_USE_NAMESPACE'),defined('BIND_MODULE'),CONTROLLER_PATH);
	}
	if (class_exists($class)) {
		return new $class();
	} else {
...
function cookie($name = '', $value = '', $option = null)
{
	...
	$name = 'NOBLES_' . str_replace('.', '_', $name);
	if ('' === $value) {
		...
		if (is_null($value)) {
			...
			setcookie($name, $value, intval($expire), $config['path'], $config['domain'], $config['secure'], $config['httponly']);
			$_COOKIE[$name] = $value;
		}
	}
	return null;
