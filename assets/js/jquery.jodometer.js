...
$.fn.jOdometer = function(settings) {
  ...
  // return this.each(function(){
		...
		var scope = $(this); // business the actual counter
		var $this = this;
		...
		// to business the digits of the number
		var integersArray = [];
		var decimalsArray = [];
    ...
    var realLength = parseInt(integers).toString().length;
      ...
      if (settings.formatNumber && i>0 && i%3==0) {
      		//if((i+1) > realLength) {
      		//	$(this).append('<div style="position:absolute; width:'+settings.widthDot+'px; height:'+settings.heightNumber+'px; background:url('+settings.numbersImage+') no-repeat center -4007px; right:'+( i*settings.widthNumber + numberOfDecimals*settings.widthNumber + widthDot + commaExtraWidth + settings.offsetRight + numberOfDecimals*settings.spaceNumbers + i*settings.spaceNumbers + settings.spaceNumbers )+'px;" class="jodometer_dot"></div>');
      		//} else {
      		//	$(this).append('<div style="position:absolute; width:'+settings.widthDot+'px; height:'+settings.heightNumber+'px; background:url('+settings.numbersImage+') no-repeat center -3740px; right:'+( i*settings.widthNumber + numberOfDecimals*settings.widthNumber + widthDot + commaExtraWidth + settings.offsetRight + numberOfDecimals*settings.spaceNumbers + i*settings.spaceNumbers + settings.spaceNumbers )+'px;" class="jodometer_dot"></div>');
      		//}      	
		...
		function updatePosition(col, newDigit, oldDigit){
			if(newDigit != oldDigit){
				...
				if( col.hasClass('jodometer_integer_4') || col.hasClass('jodometer_integer_5') || col.hasClass('jodometer_integer_6') || col.hasClass('jodometer_integer_7') ){
					$('.jodometer_integer_4, .jodometer_integer_5, .jodometer_integer_6, .jodometer_integer_7').css({
						'top': -3214
					});
				}else{
					if (newDigit == 0){
