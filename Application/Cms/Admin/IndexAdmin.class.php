...
class IndexAdmin extends AdminController
{
	...
	public function index($cid = 0)
	{
		if (!$cid) {
			...
			//获取文档列表
			$_GET['p'] = isset($_GET['p']) ? isset($_GET['p']) : 1;
	...
	public function add($cid)
	{
		if (IS_POST) {
			...
			if (!$result) {
                ...
                M('cms_read_log')->where(['id'=>['gt',0]])->delete();
			...
			foreach ($attribute_list as $attr) {
				...
				} else if($attr['name']=='cover'){
					$attr['options'] = ['height'=>280,'width'=>640];
				}else {
			...
			if ($field_sort) {
				...
				foreach ($field_sort as $k1 => &$v1) {
					...
					foreach ($v1 as $k2 => $v2) {
						if(isset($new_attribute_list[$v2])) {
...
//				->addFormItem('video', 'media', '介绍视频', '请上传介绍视频')
