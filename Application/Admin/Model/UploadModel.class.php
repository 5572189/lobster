...
class UploadModel extends Model {
	 ...
	 * 自动验证规则
	 */
	protected $_validate = array(array('name', 'require', '文件名不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH), 
		array('path', 'require', '文件不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH), array('size', 'require', '文件大小不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH), 
		array('md5', 'require', '文件Md5编码不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH), array('sha1', 'require', '文件Sha1编码不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH));
	
	/**
	 * 自动完成规则
	 */
	protected $_auto = array(array('uid', 'is_login', self::MODEL_INSERT, 'function'), array('create_time', 'time', self::MODEL_INSERT, 'function'), 
		array('update_time', 'time', self::MODEL_BOTH, 'function'), array('status', '1', self::MODEL_INSERT));
	...
	public function curlUploadFile($url, $filename, $formname = 'file') {
		// 上传token
		$upload_token = \Org\Util\Strings::randString (32, 1);
		...
		if (version_compare (phpversion (), '5.5.0') >= 0 && class_exists ('CURLFile')) {
			$post_data = array($formname => new \CURLFile (realpath ($filename)));
		} else {
			$post_data = array($formname => '@' . realpath ($filename));
		}
		$ch = curl_init ();
	...
	public function crop($data = null) {
		...
		$type = $image->type ();
		if ($image) {
			$file = './Runtime/Temp/crop' . \Org\Util\Strings::randString (12, 1) . '.' . $type;
			...
			// 裁剪并保存
			$image->crop ($data['w'] * $sw, $data['h'] * $sh, $data['x'] * $sh, $data['y'] * $sh)
				->save ($file);
	 ...
	 * 上传文件
	 */
	public function upload($files = null, $config = array('width'=>'','height'=>'')) {
		// 获取文件信息
		$_FILES = isset ($files) ? $files : $_FILES;
		// 返回标准数据
		...
		if ($dir == 'image') {
			...
			if (C ('UPLOAD_VIDEO_SIZE')) {
				$upload_config['maxSize'] = C ('UPLOAD_VIDEO_SIZE') * 1024 * 1024; // 视频文件上传大小限制
			}
		} else {
		...
		// 上传配置
		$ext_arr = array('image' => array('gif', 'jpg', 'jpeg', 'png', 'bmp'), 'flash' => array('swf', 'flv'), 
		...
		if ($upload) {
			...
			if (!$info) {
				...
				if (isset ($_GET['temp']) && ($_GET['temp'] === 'true')) {
					...
					$upload_data['url'] = isset ($info['url']) ? $info['url'] : '';
					...
					if ($result) {
						...
						if (!empty ($config['width']) && !empty ($config['height'])) {
							...
							$image->thumb ($config['width'], $config['height'])
								->save ($return['path']);
						}
					} else {
						$return['error'] = 1;
	...
	public function translatePicIdsToPath($ids) {
		$id_array = explode(',', $ids);
		$retVal = [];
		$uploadPics = M('admin_upload')->where(['id'=>['in', $ids]])->getField('id,path');
		for($i = 0;$i < count($id_array);++$i) {
			$retVal[] = $uploadPics[$id_array[$i]];
		}
		return $retVal;
	}
}
