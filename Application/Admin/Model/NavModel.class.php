...
class NavModel extends Model {
	 ...
	 * 自动验证规则
	 */
	protected $_validate = array(array('name', '', '导航名称已存在', self::MUST_VALIDATE, 'unique', self::MODEL_BOTH), 
		array('name', '/^[\w]+$/', '名称必须是纯英文，不包含下划线、空格及其他字符', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH), array('title', 'require', '导航标题不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH), 
		array('type', 'require', '导航类型不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH), array('url', '0,255', '链接长度为0-25个字符', self::EXISTS_VALIDATE, 'length', self::MODEL_BOTH));
	
	/**
	 * 自动完成规则
	 */
	protected $_auto = array(array('value', 'get_value_by_type', self::MODEL_BOTH, 'callback'), array('create_time', 'time', self::MODEL_INSERT, 'function'), 
	...
	public function getParentNav($id) {
		...
		$con['status'] = 1;
		$nav_list = $this->where ($con)
			->field (true)
			->select ();
		$current_nav = $this->field (true)
			->find ($id); // 获取当前导航的信息
		$result[] = $current_nav;
		$pid = $current_nav['pid'];
		while ( true ) {
