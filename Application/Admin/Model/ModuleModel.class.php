...
class ModuleModel extends Model {
	 ...
	 * 自动验证规则
	 */
	protected $_validate = array(array('name', 'require', '模块名称不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH), array('name', '', '该模块已存在', self::MUST_VALIDATE, 'unique', self::MODEL_BOTH), 
		array('title', 'require', '模块标题不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH), array('description', 'require', '模块描述不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH), 
		array('developer', 'require', '模块开发者不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH), array('version', 'require', '模块版本不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH), 
		array('admin_menu', 'require', '模块菜单节点不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH));
	
	/**
	 * 自动完成规则
	 */
	protected $_auto = array(array('create_time', 'time', self::MODEL_INSERT, 'function'), array('update_time', 'time', self::MODEL_BOTH, 'function'), array('sort', '0', self::MODEL_INSERT), 
			...
			if (isset ($val['url'])) {
				...
				switch ($dirCont) {
					case 3 : // 【Cms/Index/index(文章管理)】
						...
						if ($config_url === $current_url || strlen (strrpos ($current_url, $config_url))) {
							$result = $val;
						}
						break;
	...
	public function getAllMenu() {
		...
		if (!$menu_list) {
			$con['status'] = 1;
			$system_module_list = $this->where ($con)
	...
	public function getParentMenu($current_menu = '', $module_name = MODULE_NAME) {
		...
		$admin_menu = $this->cache (true)
			->getFieldByName ($module_name, 'admin_menu');
		$admin_menu = json_decode ($admin_menu, true);
