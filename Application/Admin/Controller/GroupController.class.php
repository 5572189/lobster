...
class GroupController extends AdminController {
	...
	public function index() {
		...
		$builder->setMetaTitle ('部门列表')
			...
			->alterTableData (array('key' => 'id', 'value' => '1'), array('right_button' => '<a class="label label-warning">超级管理员无需操作</a>'))
			->display ();
	}
	...
	public function add() {
		if (IS_POST) {
			...
			// 获取功能模块的后台菜单列表
			$tree = new Tree ();
			$moule_list = D ('Module')->where (array('status' => 1))
