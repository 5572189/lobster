...
class ModuleController extends AdminController {
	...
	public function checkDependence($dependences) {
		if (is_array ($dependences)) {
			foreach ( $dependences as $key => $val ) {
				$con['name'] = $key;
				$module_info = D ('Module')->where ($con)
					->find ();
				if (!$module_info) {
	...
	public function install_before($name) {
		...
		$builder->setMetaTitle ('准备安装模块')
			->setPostUrl (U ('install'))
	...
	public function uninstall_before($id) {
		// 使用FormBuilder快速建立表单页面。
		$builder = new \Common\Builder\FormBuilder ();
		$builder->setMetaTitle ('准备卸载模块')
			->setPostUrl (U ('uninstall'))
	...
	public function updateInfo($id) {
		...
		// 获取用户中心导航
		if (!empty($config_info['user_nav'])) {
			$data['user_nav'] = json_encode ($config_info['user_nav'], JSON_UNESCAPED_UNICODE);
		} else {
			$data['user_nav'] = '';
		}
