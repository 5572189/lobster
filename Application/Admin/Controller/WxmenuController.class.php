...
class WxmenuController extends AdminController {
	...
	public function index() {
		...
		$builder->setMetaTitle ("列表")
			->addTopButton ("addnew")
			->addTopButton ("self", ['title' => '生成菜单', 'href' => U ('build'), 'class' => 'btn btn-primary'])
			...
			->addRightButton ("self", ['title' => '添加子菜单', 'href' => U ('add', ['fid' => '__data_id__'])])
			->addRightButton ("edit")
	...
	public function add($fid = 0) {
		if (IS_POST) {
			...
			$topMenu = D ('wxmenu')->order ('sort ASC,id ASC')
				->select ();
			if (!$fid) {
				$topMenu = [];
			} else {
			...
			$builder->setMetaTitle ('新增')
				...
				->setFormData (['sort' => D ('wxmenu')->MAX ('sort') + 1, 'fid' => $fid])
	...
	public function build() {
		$wx = [];
		$menus = D ('wxmenu')->order ('sort ASC, id ASC')
			->select ();
	...
	private function getWxmenuByRow($row, $sub_button = []) {
		...
		return ["type" => $row['type'] ? 'click' : 'view', "name" => $row["name"], "sub_button" => $sub_button, $field => $row['url']];
	}
}
