...
class PublicController extends CommonController {
	...
	public function login() {
		$host = $_SERVER['HTTP_HOST']; // ns.idea580.com
		$this->assign ('host', $host);
		if (IS_POST) {
			$username = I ('username');
			$password = I ('password');
			
			// if ($host != "ns.idea580.com") {
			// // 图片验证码校验
			...
			$account_info = $account_object->where ($where)
				->find ();
			if (!$account_info) {
			...
			if (0 < $account_info['uid'] && $account_info['uid'] === $uid) {
				$this->success ('登录成功！', U ('Cms/Index/index'));
			} else {
				$this->logout ();
			}
		} else {
	...
	public function verify($vid = 1) {
		$verify = new Verify (['useCurve' => false, 'useNoise' => false, 'fontSize' => 36]);
		$verify->length = 4;
		$verify->entry ($vid);
	}
