...
class AddonController extends AdminController {
	...
	public function hooks() {
		...
		if (isset ($map['keyword'])) {
			$map['name'] = ['like', "{$map['keyword']}"];
			unset ($map['keyword']);
		}
		
		list ($data_list, $page, $model_object) = $this->lists ('admin_hook', $map, 'id DESC');
		int_to_string ($data_list, ['type' => C ('HOOKS_TYPE')]);
		$builder = new \Common\Builder\ListBuilder ();
		$builder->setMetaTitle ('钩子列表')
			->addTopButton ('addnew', ['href' => U ('addHook')])
			->setSearch ('名称', U ())
			->addTableColumn ('id', 'ID')
			->addTableColumn ('name', '名称')
			->addTableColumn ('description', '描述')
			->addTableColumn ('status', '状态', 'status')
			->addTableColumn ('type_text', '类型')
			->addTableColumn ('right_button', '操作', 'btn')
			->addRightButton ('edit', ['href' => U ('editHook', ['id' => '__data_id__'])])
			->addRightButton ('forbid', ['model' => 'hook'])
			->addRightButton ('delete', ['model' => 'hook'])
	...
	public function addHook() {
		if (IS_POST) {
			$model_object = D ('hook');
			$data = $model_object->create (I ('post.'));
			if ($data) {
				$id = $model_object->add ($data);
				if ($id) {
					$this->success ("添加成功", U ('hooks'));
				} else {
					$this->error ("添加失败" . $model_object->getError ());
				}
			} else {
			...
			$builder->setMetaTitle ("添加")
				->setPostUrl (U (''))
				->addFormItem ("name", "text", "名称", "（需要在程序中先添加钩子，否则无效）")
				->addFormItem ('description', 'textarea', '钩子描述', '（钩子的描述信息）')
				->addFormItem ('type', 'select', '类型', '', C ('HOOKS_TYPE'))
				->setFormData (['type' => 1])
				->display ();
		}
	}
	...
	public function editHook($id) {
		...
		if (IS_POST) {
			...
			if ($data) {
				...
				if (false !== $id) {
					$this->success ("编辑成功", U ('hooks'));
				} else {
					$this->error ("编辑失败" . $model_object->getError ());
				}
			} else {
				$this->error ($model_object->getError ());
			}
	...
	public function config() {
		if (IS_POST) {
			...
			if ($addon['custom_config']) {
				...
				// 使用FormBuilder快速建立表单页面。
				$builder = new \Common\Builder\FormBuilder ();
				$builder->setMetaTitle ('插件设置')
					->setPostUrl (U ('config'))
					->addFormItem ('id', 'hidden', 'ID', 'ID')
					->setExtraItems ($addon['config'])
					->setFormData ($addon)
					->display ();
			}
		}
	...
	public function install() {
		...
		if ($addon_object->add ($data)) {
			...
			$addon_object->where ("name='{$addon_name}'")
				->save ($config);
	...
	public function uninstall() {
		...
		S ('hooks', null);
		$delete = $addon_object->where ("name='{$db_addons['name']}'")
			->delete ();
	...
	public function adminList($name, $tab = 1) {
		...
		if ($addon->custom_adminlist) {
			...
			foreach ( $admin_list as $key => $val ) {
				...
				$tab_list[$key]['href'] = U ('Admin/Addon/adminList', array('name' => $name, 'tab' => $key));
			}
			...
			if ($param) {
				...
				// 获取数据列表
				$p = !empty ($_GET["p"]) ? $_GET['p'] : 1;
				$data_list = M ($param['model'])->page ($p, C ('ADMIN_PAGE_ROWS'))
					->where ($map)
					->field (true)
					->order ($param['order'])
					->select ();
				$page = new Page (M ($param['model'])->where ($map)
					->count (), C ('ADMIN_PAGE_ROWS'));
				
				// 使用Builder快速建立列表页面。
				$builder = new \Common\Builder\ListBuilder ();
				$builder->setMetaTitle ($addon->info['title'])
					->AddTopButton ('addnew', array('href' => U ('Admin/Addon/adminAdd', array('name' => $name, 'tab' => $tab))))
					->AddTopButton ('resume', array('model' => $param['model']))
					->AddTopButton ('forbid', array('model' => $param['model']))
					->setSearch ('请输入关键字', U ('Admin/Addon/adminList', array('name' => $name, 'tab' => $tab)))
					->SetTabNav ($tab_list, $tab)
					->setTableDataList ($data_list)
				...
				$builder->addTableColumn ('right_button', '操作', 'btn')
					...
					->addRightButton ('forbid', array('model' => $param['model']))
					->addRightButton ('delete', array('model' => $param['model']))
					->display ();
			} else {
	...
	public function adminAdd($name, $tab) {
		...
		if ($param) {
			if (IS_POST) {
				...
				// 使用FormBuilder快速建立表单页面。
				$builder = new \Common\Builder\FormBuilder ();
				$builder->setMetaTitle ('新增数据')
					->setPostUrl (U ('Admin/Addon/adminAdd', array('name' => $name, 'tab' => $tab)))
	...
	public function adminEdit($name, $tab, $id) {
		...
		if ($param) {
			if (IS_POST) {
				...
				// 使用FormBuilder快速建立表单页面。
				$builder = new \Common\Builder\FormBuilder ();
				$builder->setMetaTitle ('编辑数据')
					->setPostUrl (U ('admin/addon/adminedit', array('name' => $name, 'tab' => $tab)))
					->addFormItem ('id', 'hidden', 'ID', 'ID')
					->setExtraItems ($param['field'])
