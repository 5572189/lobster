...
class ConfigController extends AdminController {
	...
	public function index($group = 1) {
		...
		$data_list = $config_object->page ($p, C ('ADMIN_PAGE_ROWS'))
			->where ($map)
			->order ('sort asc,id asc')
			->select ();
		$page = new Page ($config_object->where ($map)
		...
		$builder->setMetaTitle ('配置列表')
			->addTopButton ('addnew', ['href' => U ('add', ['group' => I ('group', 1)])])
			->addTopButton ('resume')
			->addTopButton ('forbid')
			->addTopButton ('delete')
		...
		if (IS_POST) {
			...
			if ($data) {
				if ($config_object->add ($data)) {
					...
					$this->success ('新增成功', U ('index', ['group' => I ('group', 1)]));
				} else {
					$this->error ('新增失败');
				}
			} else {
			...
			$builder->setMetaTitle ('新增配置')
				->setPostUrl (U ('add', ['group' => I ('group', 1)]))
	...
	public function edit($id) {
		if (IS_POST) {
			...
			$config = D ('Config')->find ($id);
			
			// 使用FormBuilder快速建立表单页面。
			$builder = new \Common\Builder\FormBuilder ();
			$builder->setMetaTitle ('编辑配置')
				->setPostUrl (U ('edit', ['group' => $config['group']]))
