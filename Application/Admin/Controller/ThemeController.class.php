...
class ThemeController extends AdminController {
	...
	public function install($name) {
		// 获取当前主题信息
		$config_file = realpath ('./Theme/' . $name) . '/' . D ('Theme')->install_file ();
		if (!$config_file) {
			$this->error ('安装失败');
		}
	...
	public function setCurrent($id) {
		...
		if ($theme_info) {
			// 当前主题current字段置为1
			$map['id'] = array('eq', $id);
			$result1 = $theme_object->where ($map)
				->setField ('current', 1);
			if ($result1) {
				// 其它主题current字段置为0
				$map = array();
				$map['id'] = array('neq', $id);
				if ($theme_object->where ($map)
					->count () == 0) {
					$this->success ('前台主题设置成功！');
				}
				$con['id'] = array('neq', $id);
				$result2 = $theme_object->where ($con)
					->setField ('current', 0);
				if ($result2) {
					$this->success ('前台主题设置成功！');
				} else {
