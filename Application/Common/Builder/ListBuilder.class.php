		...
		foreach ($this->_table_data_list as &$data) {
			...
			if ($this->_right_button_list) {
				foreach ($this->_right_button_list as $right_button) {
					...
					if (isset($right_button['type']) && ($right_button['type'] === 'forbid' || $right_button['type'] === 'hide')) {
			...
			foreach ($this->_table_column_list as &$column) {
				switch ($column['type']) {
					...
					case 'byte':
						...
						break;
					case 'icon':
						$data[$column['name']] = isset($data[$column['name']]) ? '<i class="fa ' . $data[$column['name']] . '"></i>' : '';
						break;
					case 'date':
					...
					case 'datetime':
						...
						break;
					case 'time':
						$data[$column['name']] = $data[$column['name']] ? time_format($data[$column['name']]) : '';
						break;
					case 'avatar':
						$data[$column['name']] = '<img style="width:40px;height:40px;" src="' . get_cover($data[$column['name']], 'avatar') . '">';
						break;
					case 'picture':
